...
import { useClientIps } from "@/hooks/useAdmin";
import type { Tables } from "@/integrations/supabase/types";

type FirewallRule = Tables<"firewall_rules">;

interface AdminRuleFormDialogProps {
  open: boolean;
  onClose: () => void;
...
export function AdminRuleFormDialog({ open, onClose, onSubmit, editRule, loading, userId }: AdminRuleFormDialogProps) {
  const [form, setForm] = useState(defaults);
  const { data: clientIps } = useClientIps(userId);
  ...
  useEffect(() => {
    if (editRule) {
      ...
      setForm({
        ...defaults,
        destination_ip: clientIps?.[0]?.ip_address || "0.0.0.0/0",
      });
    }
  }, [editRule, open, clientIps]);
      ...
      <DialogContent className="glass border-border/50 max-w-lg max-h-[90vh] overflow-y-auto rounded-2xl">
        <DialogHeader>
          <DialogTitle className="text-foreground font-semibold">
            {editRule ? "Editare Regulă (Admin)" : "Regulă Nouă (Admin)"}
          </DialogTitle>
        </DialogHeader>
